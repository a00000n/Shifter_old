...
import static alon.com.shifter.base_classes.Consts.Fb_Dirs.MGR_SEC_USER_REQUESTS;
...
public class FirebaseUtil implements Consts {
    ...
    /**
     * A {@link FirebaseStorage} instance;
     */
    ...
    /**
     * Singleton method to get {@link #mStorage}.
     *
     * @return {@link #mStorage}.
     */
    public static StorageReference getStorage(String dir) {
        return FirebaseStorage.getInstance().getReference(dir);
    }
...
/*        getDatabase(Fb_Dirs.USERS).child(uid).addListenerForSingleValueEvent(new ValueEventListener() {
    ...
    public static void getUsersUIDsForManager(final FinishableTaskWithParams task) {
        getDatabase(MGR_SEC).child(getUser().getCode()).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
    ...
    public static void acceptedUser(final BaseUser acceptedUser) {
        ...
        final FinishableTaskWithParams mTask = new FinishableTaskWithParams() {
            ...
            public void onFinish() {
                ...
                getDatabase(MGR_SEC).child(getUser().getCode()).child(Fb_Keys.MGR_SEC_USERS_ACCEPTED).setValue(mAcceptedUsers);
            }
        };
        getDatabase(MGR_SEC).child(getUser().getCode()).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
    ...
    public static void startWaitForApproval(final TaskResult task) {
        ChildEventListener approvalEvent = new ChildEventListener() {
            @Override
            ...
            public void onChildChanged(DataSnapshot dataSnapshot, String s) {
                if (dataSnapshot.getKey().equals(getUser().getCode())) {
                    for (DataSnapshot snap : dataSnapshot.getChildren()) {
                        if (snap.getKey().equals(getUser().getUID()) && (boolean) snap.getValue()) {
                            finish();
                            return;
                        }
                    }
                }
                if (dataSnapshot.getKey().equals(Fb_Keys.MGR_SEC_USERS_ACCEPTED)) {
                    if (dataSnapshot.getValue().toString().contains(getUser().getUID()))
                        finish();
                }
//                boolean condition1 = (dataSnapshot.getKey().equals(Fb_Keys.MGR_SEC_USERS_ACCEPTED) && dataSnapshot.getValue().toString().contains(getUser().getUID()));
//                boolean condition2 = (dataSnapshot.getKey().equals(getUser().getUID()) && (boolean) dataSnapshot.getValue());
            }
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                Log.e(TAG, "onCancelled: Failed waiting for approval.", databaseError.toException());
            }

            private void finish() {
                try {
                    getDatabase(MGR_SEC).child(getUser().getCode()).removeEventListener(this);
                } catch (Exception ex) {
                    Log.i(TAG, "onChildChanged: " + ex.getMessage());
                } finally {
                    try {
                        getDatabase(MGR_SEC_USER_REQUESTS).removeEventListener(this);
                    } catch (Exception ex) {
                        Log.i(TAG, "onChildChanged: " + ex.getMessage());
                    } finally {
                        task.onSucceed();
        ...
        getDatabase(Fb_Dirs.MGR_SEC).child(getUser().getCode()).addChildEventListener(approvalEvent);
        getDatabase(Fb_Dirs.MGR_SEC_USER_REQUESTS).addChildEventListener(approvalEvent);
    }
}
