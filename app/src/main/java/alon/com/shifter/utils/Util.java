...
import alon.com.shifter.dialog_fragments.PhoneNumberDialogFragment;
...
import static alon.com.shifter.wrappers.WrapperBase.WRAPPER_DIALOG_FRAGMENT_PHONENUMBER_ID;
...
//                new BackgroundInfoFetcher().execute((Activity) mCon);
    ...
    public void writePref(Context con, Object... data) {
        ...
        for (int i = 0; i < data.length; i += 2) {
            ...
            else if (data[next] instanceof Long)
                mEditor.putLong(key, (Long) data[next]);
            else
...
//    }
    ...
    private void getPhoneNumber(Context context) {
        if (readPref(context, Pref_Keys.USR_NUMBER, Strings.NULL).equals(Strings.NULL)) {

            PhoneNumberDialogFragment frag = new PhoneNumberDialogFragment();

            OnClickWrapper clickWrapper = new OnClickWrapper() {
                @Override
                public void onClick(View v) {
                    PhoneNumberDialogFragment frag = (PhoneNumberDialogFragment) getWrapperParam(WRAPPER_DIALOG_FRAGMENT_PHONENUMBER_ID);
                    Context context = (Context) getWrapperParam(WRAPPER_CONTEXT_ID);
                    String number = frag.getNumber();
                    if (number.isEmpty() || number.length() != 10) {
                        Toast.makeText(context, R.string.phone_num_dialog_err_not_correct_number, Toast.LENGTH_SHORT).show();
                        return;
                    }
                    new NumberFetcher().execute(number, context, frag);
                }
            };
            clickWrapper.setWrapperParam(WRAPPER_DIALOG_FRAGMENT_PHONENUMBER_ID, frag).setWrapperParam(WRAPPER_CONTEXT_ID, context);

            OnClickWrapper cancelWrapper = new OnClickWrapper() {
                @Override
                public void onClick(View v) {
                    PhoneNumberDialogFragment frag = (PhoneNumberDialogFragment) getWrapperParam(WRAPPER_DIALOG_FRAGMENT_ID);
                    Context context = (Context) getWrapperParam(WRAPPER_CONTEXT_ID);
                    Util util = (Util) getWrapperParam(WRAPPER_UTIL_ID);

                    util.writePref(context, Pref_Keys.USR_NUMBER, Strings.NULL);
                    frag.dismiss();
                    System.exit(0);
                }
            };
            cancelWrapper.setWrapperParam(WRAPPER_DIALOG_FRAGMENT_PHONENUMBER_ID, frag).setWrapperParam(WRAPPER_CONTEXT_ID, context).setWrapperParam(WRAPPER_UTIL_ID, this);

            frag.setTitle(context.getString(R.string.phone_num_dialog_please_input_phone_number));
            frag.setInfo(context.getString(R.string.phone_num_dialog_phone_number_explained));
            frag.setStyle(DialogFragment.STYLE_NO_TITLE, 0);
            frag.setSubmitListener(clickWrapper);
            frag.setCancelListener(cancelWrapper);
            frag.show(((Activity) context).getFragmentManager(), DialogFragment_Keys.NUMBER_FETCHER_DIALOG);
        }
    }
    ...
    /**
     * Converts a {@link Serializable} object to a byte array.
     *
     * @param obj
     ...
     * @throws IOException
     */
    public static byte[] seralizeObject(Serializable obj) throws IOException {
        ByteArrayOutputStream outputBytes = new ByteArrayOutputStream();
        ObjectOutputStream objectStream = new ObjectOutputStream(outputBytes);
        objectStream.writeObject(obj);
        return outputBytes.toByteArray();
    }

    /**
     * Converts a byte array to a {@link Serializable} object.
     *
     * @param bytes
     *         - the byte array.
     *
     ...
     */
    public static Object deseralizeObject(byte[] bytes) throws IOException, ClassNotFoundException {
        ByteArrayInputStream input = new ByteArrayInputStream(bytes);
        ObjectInputStream objectStream = new ObjectInputStream(input);
        return objectStream.readObject();
    }

    public static String getArrangementString(Arrangement arrg) {
        return null;
    }
    ...
    private class NumberFetcher extends AsyncTask<Object, Void, Void> {
        ...
        protected Void doInBackground(Object... params) {
            ...
            final DialogFragment mInsertNumber = (DialogFragment) params[2];
            final ProgressDialogFragment[] mVerDialog = new ProgressDialogFragment[1];
            ...
            ((Activity) mCon).runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    mVerDialog[0] = new ProgressDialogFragment();
                    mVerDialog[0].setTitle(mCon.getString(R.string.phone_num_dialog_checking_phone_number));
                    mVerDialog[0].setMessage(mCon.getString(R.string.phone_num_dialog_verifing_phone_num));
                    mVerDialog[0].setCancelable(false);
                    mVerDialog[0].show(((Activity) mCon).getFragmentManager(), DialogFragment_Keys.NUMBER_VERIFICATION_DIALOG);
                    mVerDialog[0].setStyle(DialogFragment.STYLE_NO_TITLE, 0);
