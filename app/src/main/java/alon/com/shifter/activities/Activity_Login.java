...
import alon.com.shifter.dialog_fragments.ConnectionProgressDialogFragment;
...
import alon.com.shifter.wrappers.TaskResultWrapper;
...
public class Activity_Login extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ConnectionProgressDialogFragment frag = new ConnectionProgressDialogFragment();
        ...
        OnClickWrapper wrapper = new OnClickWrapper() {
            ...
            public void onClick(View v) {
                ...
                mUtil.writePref((Context) getWrapperParam(WRAPPER_CONTEXT_ID), Pref_Keys.LOG_PERMA_LOGIN, false);
                mUtil.writePref((Context) getWrapperParam(WRAPPER_CONTEXT_ID), Pref_Keys.LOG_PERMA_LOGIN_PASS, Strings.NULL);
                mUtil.writePref((Context) getWrapperParam(WRAPPER_CONTEXT_ID), Pref_Keys.LOG_PERMA_LOGIN_EMAIL, Strings.NULL);
                init();
            }
        };
        wrapper.setWrapperParam(WRAPPER_DIALOG_FRAGMENT_ID, frag).setWrapperParam(WRAPPER_CONTEXT_ID, this);

        frag.setTitle(getString(R.string.connecting_msg));
        frag.setMessage(getString(R.string.please_wait));
        frag.setOnCancelListener(wrapper);
        frag.setStyle(DialogFragment.STYLE_NO_TITLE, 0);
        frag.setCancelable(false);
        frag.show(getFragmentManager(), DialogFragment_Keys.CONNECTION_KEY);
                    ...
                    TaskResultWrapper wrapper = new TaskResultWrapper() {
                        ...
                        public void onSucceed() {
                            if (FirebaseUtil.getUser().isManager())
                                mUtil.changeScreen((Activity) getWrapperParam(WRAPPER_CONTEXT_ID), Activity_Shifter_Main_Manager.class);
                            else
                                mUtil.changeScreen((Activity) getWrapperParam(WRAPPER_CONTEXT_ID), Activity_Shifter_Main_User.class);
                            AsyncWrapper wrapper = new AsyncWrapper() {
                                @Override
                                protected Void doInBackground(Void... params) {
                                    try {
                                        setGate(Consts.Fc_Keys.LOGIN_OR_REGISTER_FINISHED, true);
                                        Linker linker = Linker.getLinker((Activity) getWrapperParam(WRAPPER_CONTEXT_ID), Consts.Linker_Keys.TYPE_INFO_FETCHER);
                            ...
                            wrapper.setWrapperParam(WRAPPER_CONTEXT_ID, getWrapperParam(WRAPPER_CONTEXT_ID));
                            wrapper.execute();
                        }
                    };
                    wrapper.setWrapperParam(WRAPPER_CONTEXT_ID, params[0]);
                    FirebaseUtil.getUserFromDatabase(wrapper);
