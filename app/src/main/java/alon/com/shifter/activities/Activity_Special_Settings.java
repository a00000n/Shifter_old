...
import static alon.com.shifter.wrappers.WrapperBase.WRAPPER_SPEC_SETTINGS_ID;
import static alon.com.shifter.wrappers.WrapperBase.WRAPPER_STRING_SHIFT_ID;
import static alon.com.shifter.wrappers.WrapperBase.WRAPPER_STRING_SPEC_SETTINGS_ID;
...
public class Activity_Special_Settings extends BaseActivity {
    ...
    protected void setupUI() {
        String mCompleteSpecSettingInfo = mUtil.readPref(this, Pref_Keys.USR_SPEC_SETTINGS_RESTS_AND_NAMES, Strings.NULL).toString();
    ...
    public void onClick(View v) {
        if (v.getId() == mDone.getId()) {
            ...
            String specSettingsString = mUtil.readPref(this, Pref_Keys.USR_SPEC_SETTINGS_RESTS_AND_NAMES, Strings.NULL).toString();
            ...
            if (specSettingsString.equals(Strings.NULL) || shifts.equals(Strings.NULL)) {
                alon.com.shifter.wrappers.FinishableTaskWrapper wrapper = new alon.com.shifter.wrappers.FinishableTaskWrapper() {
                    ...
                    public void onFinish() {
                        count++;
                        if (count == 2) {
                            finishOnClick((SpecSettings) getWrapperParam(WRAPPER_SPEC_SETTINGS_ID), (String) getWrapperParam(WRAPPER_STRING_SPEC_SETTINGS_ID), (String) getWrapperParam(WRAPPER_STRING_SHIFT_ID));
                        }
                    }
                };
                wrapper.setWrapperParam(WRAPPER_SPEC_SETTINGS_ID, settings);
                wrapper.setWrapperParam(WRAPPER_STRING_SPEC_SETTINGS_ID, specSettingsString);
                wrapper.setWrapperParam(WRAPPER_STRING_SHIFT_ID, shifts);
    ...
    private void finishOnClick(SpecSettings specSettings, String specSettingsString, String shifts) {
        ...
        AsyncWrapper wrapper = new AsyncWrapper() {
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    linker.addParam(Linker_Keys.KEY_SPEC_SETTINGS, getWrapperParam(WRAPPER_SPEC_SETTINGS_ID));
                    linker.execute();
        ...
        wrapper.setWrapperParam(WRAPPER_SPEC_SETTINGS_ID, specSettings);
        wrapper.execute();
