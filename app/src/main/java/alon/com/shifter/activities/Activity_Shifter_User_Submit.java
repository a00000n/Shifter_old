...
import static alon.com.shifter.wrappers.WrapperBase.WRAPPER_SHIFT_ID;
...
public class Activity_Shifter_User_Submit extends BaseActivity {
    ...
    protected void setupUI() {
        ...
        if (!mSettings.equals(SpecSettings.getEmpty()))
            mShiftSubmissions.setSpecSettings(mSettings, (String) mUtil.readPref(this, Pref_Keys.USR_SPEC_SETTINGS_RESTS_AND_NAMES, Strings.NULL));
        ...
        String submissionCap = (String) mUtil.readPref(this, Pref_Keys.MGR_SEC_SUBMISSION_CAP, Strings.NULL);
        if (!submissionCap.equals(Strings.NULL)) {
            String[] parts = submissionCap.split("~");
            int lastDay = Integer.parseInt(parts[0]);
            int hr = Integer.parseInt(parts[1].split(":")[0]);
            int min = Integer.parseInt(parts[1].split(":")[1]);
            Calendar cal = Calendar.getInstance();
            //TODO : change to online time.
            cal.setTimeInMillis(System.currentTimeMillis());
            int day = cal.get(Calendar.DAY_OF_WEEK);
            int currentHour = cal.get(Calendar.HOUR_OF_DAY);
            int currentMin = cal.get(Calendar.MINUTE);
            if (lastDay > day || (lastDay <= day && (currentHour == hr && currentMin > min) || (currentHour > hr)))
                mShiftSubmissions.setEnabled(false);
        }
    }
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.USR_SS_done) {
                ...
                protected Void doInBackground(Void... params) {
                    try {
                        ...
                        linker.setOnFinish(new FinishableTask() {
                            ...
                            @Override
                            public void onFinish() {
                                mFinishHandler.post(new RunnableWrapper((DialogFragment) getWrapperParam(WRAPPER_DIALOG_FRAGMENT_ID)) {
                                    @Override
                                    public void run() {
                                        mFrag.dismiss();
            ...
            wrapper.setWrapperParam(WRAPPER_SHIFT_ID, submittedShiftObject).setWrapperParam(WRAPPER_DIALOG_FRAGMENT_ID, frag);
            wrapper.execute();
        }
    }
