            ...
            case R.id.MGR_SHS_done:
                AsyncWrapper wrapper = new AsyncWrapper() {
                    ...
                    protected Void doInBackground(Void... params) {
                        try {
                            Linker linker = Linker.getLinker((Activity) getWrapperParam(WRAPPER_ACTIVITY_ID), Linker_Keys.TYPE_UPLOAD_FILE_SHIFT_HOURS);
                            linker.addParam(Linker_Keys.KEY_SHIFT_HOURS_OBJECT, getWrapperParam(WRAPPER_SHIFT_HOURS_ID));
                            linker.execute();
                        } catch (Linker.ProductionLineException | Linker.InsufficientParametersException ex) {
                            ex.printStackTrace();
                        }
                        return null;
                    }
                };
                wrapper.setWrapperParam(WRAPPER_SHIFT_HOURS_ID, mShiftHours);
                wrapper.execute();
                ...
                if (mChangeText)
                    ...
                    mUtil.changeScreen(this, Activity_Shifter_Manager_SubmissionCap.class);
                return;
        }
